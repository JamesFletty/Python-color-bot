...
-- NOTE: Matrix SoColor intermixing-rule seeds were moved to
-- seed_intermixing_rules.sql and are applied by import_stage12_research after
-- the research tables are populated. They are data-dependent (INSERT ... SELECT
-- from brand/product_line/sub_range/source_document); running them here during
-- migration matched zero rows because the research import had not happened yet,
-- so no intermixing rules were ever loaded.
