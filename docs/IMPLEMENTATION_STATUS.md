...
**Status:** Partial. API/CLI callers can now pass `consultation_id` and `stylist_id` through the PostgreSQL formula path, and persistence uses that stylist when provided.

**Scope:** Auth middleware, salon ID propagation, and consultation lifecycle before formula persist.
